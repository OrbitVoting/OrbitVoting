...
from updateData import updateData, getData, listData, getToken
...
async def checkForData():
    ...
    while not client.is_closed():
        ...
        print(delta)
        timeSpentOn = (datetime.datetime.now()-datetime.datetime.fromisoformat(getData("auto_start_time"))).seconds
        if(getData("status") == "on" and timeSpentOn > 3600*48): #turn off after 48 hours
    ...
    if ((message.channel.id in LIST_OF_CHANNELS) == False and getToken("name").lower() == "alpha"):
        ...
        if message.content == "$votecount vis on":
          ...
          await message.channel.send("I'm still here!")
            ...
            updateData("last_time", datetime.datetime.now().isoformat())
            updateData("auto_start_time",datetime.datetime.now().isoformat())
